...
4. **Estructura de pasos A/B/C/D.** El enum `paso_tipo` actual conserva la numeración lineal 1-10 vieja; el PRD v1.8+ usa Paso 0 + grupos A/B/C/D con N/A automáticos por destino (C1), por flujo de pago (B2) y por tipo de viaje (D2). Además `viajes` no tiene campo `tipo_viaje` (grupal/individual). Ojo: B1/B2 están gateados por **CRIT-01** y D2 por **CRIT-03** (`OPEN_DECISIONS.md`); el Panel de Estados (3A) hereda esos gates.
