/*
 * Primitivos locales de la dirección MARINO.
 ...
 *
 * Carácter MARINO: claro, aireado, sobrio. Navy como fill primario,
 * oro como acento puntual. FILLS SÓLIDOS, CERO degradés.
 */
...
export function Badge({
  ...
  return (
    <span
      className={cn(
        "inline-flex items-center gap-1.5 rounded-[var(--r-pill)] px-3 py-1 text-[var(--t-label)] font-semibold leading-none tracking-[var(--ls-label)]",
        className,
      )}
...
/* ── Button ──────────────────────────────────────────────────────── */
/*
 * variant="primary" → navy sólido (acción principal).
 * variant="accent"  → oro sólido (acción destacada puntual, sin degradé).
 * Ningún variant usa background-image: TODO fill plano.
 */
...
export function Button({
  ...
  const variants: Record<ButtonVariant, string> = {
    primary:
      "bg-[var(--c-brand)] text-[var(--c-ink-onbrand)] shadow-[var(--shadow-brand)] hover:bg-[var(--c-brand-700)] focus-visible:shadow-[var(--ring-focus)]",
    accent:
      "bg-[var(--c-accent)] text-[var(--c-ink-onaccent)] shadow-[var(--shadow-accent)] hover:bg-[var(--c-accent-600)] hover:text-[var(--c-ink-onbrand)] focus-visible:shadow-[var(--ring-accent)]",
    outline:
      "border border-[var(--c-border-strong)] bg-[var(--c-surface)] text-[var(--c-ink)] shadow-[var(--shadow-soft)] hover:border-[var(--c-brand-300)] hover:text-[var(--c-brand)] focus-visible:shadow-[var(--ring-focus)]",
    ghost:
      "text-[var(--c-ink-muted)] hover:bg-[var(--c-overlay)] hover:text-[var(--c-ink)] focus-visible:shadow-[var(--ring-focus)]",
  };
...
const controlState = {
  normal:
    "border-[var(--c-border-strong)] focus:border-[var(--c-brand-300)] focus:shadow-[var(--ring-focus)]",
  error:
...
export function SectionTitle({
  ...
  return (
    <div>
      {kicker && (
        <p className="text-[var(--t-label)] font-bold uppercase tracking-[var(--ls-label)] text-[var(--c-accent-600)]">
          {kicker}
        </p>
      )}
