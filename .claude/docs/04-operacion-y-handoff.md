...
| `job:purga` | Purga por retención (MIN-16): vacía los datos personales de las inscripciones fuera de plazo (90 días las ya volcadas a un alumno, 730 las que nunca se procesaron) y borra el token de las invitaciones vencidas hace más de 90 días que nadie usó. **Las filas no se borran**: queda el talón (número, estado, variante, fechas, campaña) para que las métricas no cambien hacia atrás. Es idempotente y va por lotes; imprime la base y las fechas de corte antes de escribir. La lógica está en `src/lib/jobs/purgar-inscripciones.ts` y el día que Trigger.dev esté desplegado la llama una `schedules.task` diaria (`purga-retencion`, `0 4 * * *` UTC) sin tocar el job | Una vez por mes, o cuando alguien pregunte por los plazos de la política publicada. **Escribe y no se deshace**: mirá contra qué base apunta `DATABASE_URL` | Lo lee el script (`.env.local`) |
